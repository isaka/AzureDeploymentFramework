...
var Workspace = [for (ws,index) in AVDWorkspaceInfo : {
  match: ((Global.CN == '.') || contains(array(Global.CN), ws.Name))
}]

module AVDWorkspace 'AVDWorkspace-WS.bicep' = [for (ws,index) in AVDWorkspaceInfo : if(Workspace[index].match) {
  name: 'dp${Deployment}-AVDHostPoolDeploy${ws.name}'
  params: {
